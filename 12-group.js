// 12-group.js — turns a pile of findings into something a person can act on.
//
...
//   anything smaller                -> listed deal by deal, as before
...
const ESCALATION_OPENERS = [
  (who, n, issue) => `${who} is carrying ${n} deals with the same problem: ${lowerFirst(issue)}.`,
  (who, n, issue) => `${n} of ${who}'s deals are all held up the same way: ${lowerFirst(issue)}.`,
  (who, n, issue) => `The same problem is sitting on ${n} of ${who}'s deals: ${lowerFirst(issue)}.`,
];
const GROUP_OPENERS = [
  (who, n, issue) => `${who} has ${n} deals where ${lowerFirst(issue)}.`,
  (who, n, issue) => `${n} of ${who}'s deals share one problem: ${lowerFirst(issue)}.`,
  (who, n, issue) => `On ${n} of ${who}'s deals, ${lowerFirst(issue)}.`,
];
...
function groupSentence(item) {
  ...
  const oldest = item.oldestHours ? ` The oldest has been sitting ${Math.round(item.oldestHours)} days.` : "";
  const tail = item.kind === "escalation"
    ? " A backlog this size will not clear deal by deal and needs to be planned."
    : "";
